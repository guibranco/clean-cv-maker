...
export const personalInfoSchema = z.object({
  ...
  if (data.hasExperience) {
    if (!data.experiences || data.experiences.length === 0) {
      ctx.addIssue({
        ...
        message: "At least one work experience is required when 'I have work experience' is enabled",
        path: ["experiences"]
      });
    } else {
  ...
  if (data.hasEducation) {
    if (!data.education || data.education.length === 0) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "At least one education entry is required when 'I want to add education' is enabled",
        path: ["education"]
      });
    } else {
  ...
  if (data.hasCertificates) {
    if (!data.certificates || data.certificates.length === 0) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "At least one certificate is required when 'I have certifications to add' is enabled",
        path: ["certificates"]
      });
    } else {
